# Task CLAUDE.md -- implementer run
...
is `{path, description}` per `schemas/sdd_v1.Deliverable` (tightened
2026-05-22 per Phase 3 IOQ2). Out-of-scope edits = validate_impl exit
9 (Gate D).
...
6. `{ORCHESTRATOR_ROOT}/skills/v3-1c-mcp-tools-guide.md` -- 1c-MCP
   stack overview.
7. `~/.claude/1c-development-rules.md` -- 1C infrastructure +
   validation skill surface (cf-validate, cfe-validate, meta-validate,
   form-validate, role-validate, etc.).
...
differs from `{GIT_TARGET_DIR}`; otherwise same dir, same constraint
modulo branch scope).
`{ORCHESTRATOR_ROOT}` is read-only except for `{TASK_ROOT_ABS}/`.

## MCP servers for this run

Configured in `{TASK_ROOT_ABS}/.mcp.implementer.json` (loaded via
`--mcp-config --strict-mcp-config`). Discover tool names via the
server's `tools/list` on first contact -- do NOT name tools from
memory.
...
(when status=ready in impl_metadata)

```
IMPLEMENT NEEDS_REVISION
```

(when a mandatory validation failed or files_changed exceeds scope)

```
IMPLEMENT BLOCKED
```

(when implementation cannot proceed; failures[] and
open_questions[blocker] must explain).

After the signal, the orchestrator runs
`scripts/validate-impl.ps1 -TaskId {TASK_ID}` and either marks the
task done (validate exit 0 + operator signoff in Stage 8) or
re-prompts with the failed exit code (see retry table in
...
- No force-push, branch delete, master/main push, rebase pushed
  commits, git config, filter-branch -- destructive history rewrites.
- No auto-merge of `{BRANCH_NAME}` into master/main -- operator only.
- No edits outside `sdd_metadata.stages[*].deliverables` -- Gate D.
- No edits in `{ORCHESTRATOR_ROOT}` outside `{TASK_ROOT_ABS}/` --
  Gate B.
- No assumptions: if a fact is not cited, it does not exist for you.
- No fabricated commits: a `commits=[]` `impl_metadata` is illegal --
  if you cannot commit anything, set status="blocked" and explain.
